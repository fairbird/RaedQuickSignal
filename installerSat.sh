...
if [ -f $BRANDOS ]; then
	...
	wget --no-check-certificate --content-disposition https://github.com/fairbird/RaedQuickSignal/blob/main/OSdream-Satfinder-5.7-RAED.tar.gz
	tar -xzf OSdream-Satfinder-5.7-RAED.tar.gz -C /
elif [ -f $BRANDVU ] && [ -f $BlackHole  ] || [ -f $VTI ] ; then
	echo "*** BH or VTI images ***"
	[ -d $SatfinderPATH ] && rm -rf $SatfinderPATH
	[ -d $SignalfinderPATH ] && rm -rf $SignalfinderPATH
	wget --no-check-certificate --content-disposition https://github.com/fairbird/RaedQuickSignal/blob/main/Signalfinder-2.4.tar.gz
